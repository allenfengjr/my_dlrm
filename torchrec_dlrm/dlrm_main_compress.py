        ...
        for it in itertools.count(1):
            try:
                ...
                if it % 128 == 0:
                    ...
                    accuracy_metric = metrics.Accuracy(compute_on_step=False, task='binary').to(device)
                    train_preds = torch.sigmoid(train_logits)
                    accuracy_metric(train_preds, train_labels)
                    auroc_metric(train_preds, train_labels)
                    if is_rank_zero:
                        #loss_res.append(loss.item())
                        #auroc_res.append(auroc)
                        #acc_res.append(acc)
                        #print(acc,flush=True)
                        train_auroc = auroc_metric.compute().item()
                        train_accuracy = accuracy_metric.compute().item()
                        #print(train_loss.item(),flush=True)
                        #print(train_accuracy,flush=True)
                        #print(train_auroc,flush=True)
            except StopIteration:
                if is_rank_zero:
