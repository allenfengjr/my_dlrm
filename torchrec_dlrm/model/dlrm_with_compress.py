...
class SparseArch(nn.Module):
    ...
    def forward(
        ...
        # or should I add compression there?
        for name in self.sparse_feature_names:
            sparse_values.append(sparse[name])
            # At this place, sparse values are on different devices
            #print(sparse[name].shape,sparse[name].device,sparse[name][0][0:5],name)
...
class DLRM(nn.Module):
    ...
    * num_features: number of dense features
    ...
    def forward(
        ...
        # print(embedded_sparse.shape,embedded_sparse.device,embedded_sparse[0][0][0:10])
        # should I add compression there?
        # At this place, I think every device hold a full embedded_sparse data
        # But the embedded_sparse data are different
